 ...
 sidebarLayout(
  sidebarPanel(
   ...
   selectInput("variable", "Select Variable:", choices = variable_choices,
               selected = "Index of Labour Productivity"),
   selectInput("industry", "Industry:", choices = industry_choices,
               selected = "Business sector"),
   selectInput("start_year", "Start Year:", choices = year_choices, selected = 1963),
   selectInput("end_year", "End Year:", choices = year_choices, selected = max(year_choices)),
   selectInput("index_year", "Index Year (Set to 100):", choices = year_choices, selected = 1963),
...
server <- function(input, output, session) {
 ...
 cumulative_growth_rates <- reactive({
  ...
  canada_data <- data_for_growth %>% dplyr::filter(country == "Canada")
  us_data <- data_for_growth %>% dplyr::filter(country == "United States")
